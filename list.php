...
$sql = "select * from device order by orderby desc limit ". $offset ." , ".$pageSize;
$cache = new File();
$videos = array();
if(!$videos = $cache->cacheData('index_ow_cache' .'-' . $page .'-' . $pageSize)) {
	echo 1;exit;
	try {
	...
	if($videos) {
		$cache->cacheData('index_ow_cache' .'-' . $page .'-' . $pageSize, $videos, 15);
	}
}
